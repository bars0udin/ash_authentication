...
| [`sign_in_enabled?`](#authentication-strategies-password-sign_in_enabled?){: #authentication-strategies-password-sign_in_enabled? } | `boolean` | `true` | If you do not want new users to be able to sign in using this strategy, set this to false. The sign in action and its `POST /auth/<subject>/<strategy>/sign_in` route are then not generated at all; `sign_in_tokens_enabled?` is unaffected, so a resource can still exchange a short-lived sign in token for a session. |
| [`sign_in_tokens_enabled?`](#authentication-strategies-password-sign_in_tokens_enabled?){: #authentication-strategies-password-sign_in_tokens_enabled? } | `boolean` | `true` | Whether or not to support generating short lived sign in tokens. Requires the resource to have tokens enabled. Independent of `sign_in_enabled?`. |
...
| [`brute_force_strategy`](#authentication-strategies-password-brute_force_strategy){: #authentication-strategies-password-brute_force_strategy } | `nil \| {:audit_log, atom} \| {:preparation, module}` |  | How to mitigate brute-force attempts against sign-in and password reset requests. Set to `{:audit_log, :audit_log_name}` to count recent failures by identity via the audit log add-on, or `{:preparation, MyModule}` to plug in a custom preparation. Defaults to `nil` (no protection). |
| [`audit_log_window`](#authentication-strategies-password-audit_log_window){: #authentication-strategies-password-audit_log_window } | `pos_integer \| {pos_integer, :days \| :hours \| :minutes \| :seconds}` | `{5, :minutes}` | Time window for counting failed attempts when using the `{:audit_log, ...}` brute force strategy. If no unit is provided, then `minutes` is assumed. Defaults to 5 minutes. |
| [`audit_log_max_failures`](#authentication-strategies-password-audit_log_max_failures){: #authentication-strategies-password-audit_log_max_failures } | `pos_integer` | `5` | Maximum allowed failures within the window before blocking when using the `{:audit_log, ...}` brute force strategy. Defaults to 5. |
